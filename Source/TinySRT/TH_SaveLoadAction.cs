        ...
        foreach (string memberName in memberNames) {
            if (type.GetMember(memberName, BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic) is { } memberInfos) {
                ...
                if (memberInfos.First().IsField()) {
                    dict[memberName] = type.GetFieldValue(memberName).TH_DeepCloneShared();
                }
                else {
                    dict[memberName] = type.GetPropertyValue(memberName).TH_DeepCloneShared();
                }
            }
        }
    }
        ...
        foreach (KeyValuePair<Type, Dictionary<string, object>> pair in values) {
            foreach (string memberName in pair.Value.Keys) {
                ...
                if (type.GetMember(memberName, BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic) is { } memberInfos) {
                    ...
                    if (memberInfos.First().IsField()) {
                        type.SetFieldValue(memberName, pair.Value[memberName].TH_DeepCloneShared());
                    }
                    else {
                        type.SetPropertyValue(memberName, pair.Value[memberName].TH_DeepCloneShared());
                    }
                }
            }
        }
        ...
        SafeAdd(
            (dictionary, _) => {
                foreach (Type type in simpleStaticFields.Keys) {
                    ...
                    dictionary[type] = values.TH_DeepCloneShared();
                }
            }, (dictionary, _) => {
                Dictionary<Type, Dictionary<string, object>> clonedDict = dictionary.TH_DeepCloneShared();
            ...
            (savedValues, _) => {
                foreach (EverestModule module in Everest.Modules) {
                    ...
                    if (modModuleFields.TryGetValue(moduleType, out FieldInfo[] moduleFields)) {
                        ...
                        savedValues[moduleType] = dict.TH_DeepCloneShared();
                    }
                }
            },
            (savedValues, _) => {
                Dictionary<Type, Dictionary<string, object>> clonedValues = savedValues.TH_DeepCloneShared();
                foreach (EverestModule module in Everest.Modules) {
        ...
        SafeAdd(
            (savedValues, _) => {
                ...
                foreach (EverestModule everestModule in Everest.Modules) {
                    ...
                    savedValues[settingsType] = settingsDict.TH_DeepCloneShared();
                }
            }, (savedValues, _) => {
    ...
    private static void SupportAudioMusic() {
        SafeAdd(
            (savedValues, level) => {
                ...
                savedValues[typeof(Audio)] = saved.TH_DeepCloneShared();
            },
            (savedValues, level) => {
    ...
    private static void SupportExtendedVariants() {
        ...
        SafeAdd((savedValues, _) => {
            if (Everest.Modules.FirstOrDefault(everestModule => everestModule.Metadata?.Name == "ExtendedVariantMode") is { } module &&
                ...
                savedValues[moduleType] = new Dictionary<string, object> { { "TriggerManager", triggerManager.TH_DeepCloneShared() } };
            }
        }, (savedValues, _) => {
            if (savedValues.TryGetValue(moduleType, out Dictionary<string, object> dictionary) &&
                ...
                if (module.GetFieldValue("TriggerManager") is { } triggerManager) {
                    savedTriggerManager.TH_DeepCloneToShared(triggerManager);
                }
            }
        });
        ...
        SafeAdd(
            (savedValues, _) => {
                ...
                savedValues[settingsType] = dict.TH_DeepCloneShared();
            },
            (savedValues, _) => {
